...
/** ===== 헤더(로고 + 검색 + 우측 메뉴) =====
 *  /logout/logout_board 는 "로그인 상태 페이지"라서
 *  우측을 "마이페이지/로그아웃"으로 고정합니다.
 */
function HeaderBarLoggedIn() {
  const router = useRouter();
  const [q, setQ] = useState("");

  const goHome = () => router.push("/logout");
  ...
  return (
    <header className="w-full border border-gray-300 bg-white">
      <div className="mx-auto max-w-6xl px-5 py-3 flex items-center justify-between gap-4">
        {/* 로고: 로그인 홈(/logout) */}
        <button onClick={goHome} className="flex items-center gap-3 hover:opacity-90">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-900">
          <Link href="/logout" className="inline-flex items-center gap-2 hover:opacity-80">
            <IconUser />
            <span>마이페이지</span>
          </Link>

          <button onClick={doLogout} className="inline-flex items-center gap-2 hover:opacity-80">
            <IconLogout />
            <span>로그아웃</span>
          </button>
        </div>
...
function EventCard({ title }: { title: string }) {
  return (
    <article className="w-full cursor-pointer">
      <div className="w-full aspect-[4/3] rounded-[18px] border border-gray-300 bg-white hover:bg-gray-50" />
...
/** ===== 페이지 ===== */
export default function LogoutBoardPage() {
  const router = useRouter();
        ...
        <div className="flex">
          ...
          <section className="flex-1 px-8 py-6">
            <Filters
              ...
              onCreate={() => router.push("/board/create")}
            />
