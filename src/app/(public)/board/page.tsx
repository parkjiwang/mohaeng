...
function HeaderBar() {
  ...
  const goHomeByState = () => {
    router.push(loggedIn ? "/logout" : "/login");
  };
  ...
  return (
    <header className="w-full border border-gray-300 bg-white">
      <div className="mx-auto max-w-6xl px-5 py-3 flex items-center justify-between gap-4">
        {/* 로고 */}
        <button onClick={goHomeByState} className="flex items-center gap-3 hover:opacity-90">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-900">
          {loggedIn ? (
            <>
              <Link href="/mypage" className="inline-flex items-center gap-2 hover:opacity-80">
                <IconUser />
                <span>마이페이지</span>
              </Link>
...
function Filters({
  ...
  const sigungu = useMemo(
    () => ["경산", "경주", "고령", "구미", "김천", "문경", "봉화", "상주", "성주", "안동", "영덕", "영양", "영주", "영천", "예천", "울진", "의성", "청도", "청송", "포항"],
    []
  );
